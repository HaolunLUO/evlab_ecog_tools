...
properties
   ...
   %% ---- LANG ELECS WORD LEVEL ANALYSIS ----
    langloc_wordwise % ideally this is how the results must be stored to avoid crowding the class property :/ - Kumar
    ...
    %% ---- TIME-SERIES ANALYSIS (baseline significance) ----
    s_vs_n_timeseries_ops
    s_vs_n_timeseries_sig
    s_vs_n_timeseries_sig_bip
    s_vs_n_timeseries_p_values
    s_vs_n_timeseries_p_values_bip
    s_vs_n_timeseries_clusters
    s_vs_n_timeseries_clusters_bip
...
methods
    ...
    function test_s_vs_n(obj,varargin)
        ...
        if ops.use_odd_for_inference
            fprintf(1,'> Using ODD trials for inference (even trials reserved for plotting) ...\n');
        ...
        parfor k=1:n_rep
            ...
            S_N_corr_rnd_store{k} = S_N_corr_rnd;
            %S_N_corr_rnd_all = cellfun(@(x,y) horzcat(x,y),S_N_corr_rnd_all,S_N_corr_rnd,'UniformOutput',false);

        end

        for k = 1:n_rep
            S_N_corr_rnd_all = cellfun(@(x,y) horzcat(x,y),S_N_corr_rnd_all,S_N_corr_rnd_store{k},'UniformOutput',false);
        end
        fprintf(1,'] done\n');
        ...
        obj.s_vs_n_sig = cell2table(horzcat('s_vs_n_sig',S_N_p_is_sig),'VariableNames',B.Properties.VariableNames(names_no_string));
    ...
    function lang_resp_plots(obj,varargin)
        ...
        % Determine split for get_timecourses
        if ops.use_odd_for_inference
            tc_split = 'even'; % plot even trials when odd were used for inference
        else
            tc_split = [];
        end
        ...
        [S_values_ave,S_values_ave_sem,nTrialsS] = obj.get_word_averages('words',ops.words,...
                                                            'condition',ops.S_condition_flag,...
                                                            'sessions',ops.sessions,...
                                                            'allElecs',ops.bipolarByShank,'use_odd_for_inference',ops.use_odd_for_inference);
        ...
        if ops.W_condition_flag % SWJN
            [W_values_ave,W_values_ave_sem,nTrialsW] = obj.get_word_averages('words',ops.words,...
                                                                'condition',ops.W_condition_flag,...
                                                                'sessions',ops.sessions,'use_odd_for_inference',ops.use_odd_for_inference);
    ...
    function [values,values_sem]=get_timecourses(obj,varargin)
        ...
        addParameter(p,'allElecs',true); % separate from useLangElecs
        ...
        if ops.average
            % Averaging across trials of possibly different sizes
            % Find max number of electrodes and max number of time points across all trials
            max_nElecs = max(cellfun(@(x) size(x,1), values));
            max_time = max(cellfun(@(x) size(x,2), values));
        
            values_sum = zeros(max_nElecs, max_time);
            values_sq_sum = zeros(max_nElecs, max_time);
            counts = zeros(max_nElecs, max_time);
        
            % Accumulate sums and counts for averaging
            for j = 1:length(values)
                val = values{j,1};
                sz = size(val);
                % Add val to sums and increment counts for valid positions
                values_sum(1:sz(1), 1:sz(2)) = values_sum(1:sz(1), 1:sz(2)) + val;
                values_sq_sum(1:sz(1), 1:sz(2)) = values_sq_sum(1:sz(1), 1:sz(2)) + val.^2;
                counts(1:sz(1), 1:sz(2)) = counts(1:sz(1), 1:sz(2)) + 1;
            end
        
            % Compute mean
            values_done = values_sum ./ counts;
        
            % Compute SEM: sqrt(variance / n)
            variance = (values_sq_sum - (values_sum.^2)./counts) ./ (counts - 1);
            values_sem = sqrt(variance ./ counts);
            values_sem(counts<=1) = 0; % SEM undefined with <=1 trial
        
        else
            % Return all trials linearly stacked, padding time dimension if needed
            max_time = max(cellfun(@(x) size(x,2), values));
            % Calculate total electrodes count
            total_nElecs = sum(cellfun(@(x) size(x,1), values));
            values_done = zeros(total_nElecs, max_time);
            ...
            for j = 1:length(values)
                ...
                nElecs = size(val,1);
                t = size(val,2);
                values_done(idx_curr:idx_curr+nElecs-1, 1:t) = val;
                % If t < max_time, remaining elements stay 0 (padding)
                idx_curr = idx_curr + nElecs;
            end
            values_sem = []; % no SEM when not averaging
        end
    ...
    function [values_ave,values_ave_sem,nTrials]=get_word_averages(obj,varargin)
        ...
        addParameter(p,'use_odd_for_inference', true); % even trials used here if odd were used for inference
        parse(p, varargin{:});
        ops = p.Results;
        ...
        % EVEN trials for plotting (odd were used for inference)
        if ops.use_odd_for_inference
    ...
    function plot_timecourse(obj,varargin)
        ...
        addParameter(p,'words',1:12);
        addParameter(p,'noDisplay',false);
        parse(p, varargin{:});
        ops = p.Results;
